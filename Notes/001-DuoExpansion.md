...
The silver reflection bands were replaced with smooth broad lighting and a single soft highlight, avoiding thresholded stripes on the curved rim. Geometry is unchanged. Video activation now keeps a decoded canvas poster as the texture until the first presented playback frame, so an autoplay rejection does not replace the poster with an unpresented video texture. A short seek prepares the poster before activation, then playback returns to the beginning.

### Video transport and preview lifecycle
...
P1011887.JPG (Mountain trail) replaces the dog still in default slot two and as the first-still fallback. Dog getting pets remains the first default video. Exported 2048px JPEG and 360px thumbnail omit EXIF/XMP metadata; original remains unchanged.
