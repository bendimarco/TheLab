...
The controls start hidden. Use the sliders icon to open the floating right-side inspector, and its close button, the same icon, or Escape to dismiss it. The panel has a 12 px inset and rounded corners; the demo moves into the remaining space while it slides in. On mobile, the inspector slides up from the bottom as a scrollable sheet, moving and fitting the full-width demo into the space above it with matching animation timing, and respecting the bottom safe area. The close button or Modify button dismisses it. The inspector is nonmodal so the demo remains interactive. Browser CSS approximates the native translucent controls; it does not use Apple's Liquid Glass APIs.
